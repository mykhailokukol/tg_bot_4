    ...
    if settings.STATEMENT == "pre-release":
        try:
            with open("media/invitation.mov", "rb") as file:
                text = "Рады приветствовать тебя в @TELE2_RLT_BOT\nТы здесь, а значит Питер ждет именно тебя!\nВ разделах вся необходимая информация о трансферах, проживании и конференции лидеров в Санкт-Петербурге.\nОбязательно смотри чек-лист, он поможет тебе ничего не забыть.\nНастройся на мероприятие с героями Tele2.\n"
...
async def callback_simple(
    ...
    match query.data:
        case "checklist":
            with open("media/photos/2.jpg", "rb") as file:
                await update.effective_chat.send_photo(
                    caption="Чек-лист:\n"
                    "С собой в поездку обязательно паспорт для проезда и заселения.\n"
                    "Гардероб предусмотреть по погоде: +2+10. Возможен дождь.\n"
                    "09.04 Дресс-код на ужин в ресторане Борсалино: элегантный стиль Smart Casual\n"
                    "10.04 Дресс-код на конференцию: деловой стиль Business Casual\n"
                    "11.04 На экскурсионную часть с собой паспорт. Дресс-код: свободный Casual\n"
                    "11.04 Дресс-код на гала -ужин: вечерний Cocktail",
                    photo=file,
                )
