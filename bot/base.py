    ...
    match query.data:
        ...
        case "timing_3":
            await update.effective_chat.send_message(
                ...
                "<b>13:00 – 15:00</b> Обед в So на 8 этаже в ресторане So Lounge\nОбед в Астория/Англетер в ресторане Борсалино\n\n"
    ...
    try:
        ...
        if len(info) > 0:
            for i in info:
                await update.message.reply_text(
                    f'ФИО: {i["full_name"]}\n'
                    f'Дата отбытия: {i["arrival_date"]}\n'
                    f'Время отбытия: {i["arrival_time"]}\n'
                    f'Номер самолета/поезда: {i["flight_train_number"]}\n'
                    f'Трансфер: {i["transfer"]}',
                    reply_markup=ReplyKeyboardRemove(),
                )
